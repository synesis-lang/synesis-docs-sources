...
## Passo 2: Instalar synesis2neo4j

```bash
pip install synesis2neo4j
```

Verifique a instalação:

```bash
synesis2neo4j --version
```

Deve retornar: `synesis2neo4j version 0.1.2`
...
[sync]
mode = "replace"  # Opções: replace, merge, append
batch_size = 1000

[metrics]
native = true       # Métricas básicas (Cypher)
gds = false         # Graph Data Science (requer plugin)
```
...
[sync]
mode = "replace"
batch_size = 500  # Menor para conexão cloud

[metrics]
native = true
gds = false  # GDS não disponível em AuraDB Free
```
...
```bash
synesis2neo4j --project meu_projeto.synp --config neo4j_config.toml
```
...
✓ Ontologia carregada: 15 códigos, 4 relações
...
### Visualizar Relações de um Código
...
### Ver Anotações Conectadas a um Código
...
### Top 10 Códigos Mais Frequentes
...
### Códigos que Co-ocorrem Frequentemente
...
## Passo 7: Atualizar o Grafo (Sincronização Incremental)
...
### Modo Merge (Atualização Incremental)
...
**Ficou com dúvidas?** Abra uma issue no [repositório synesis2neo4j](https://github.com/synesis-lang/synesis2neo4j) ou consulte [GitHub Discussions](https://github.com/synesis-lang/synesis/discussions).
