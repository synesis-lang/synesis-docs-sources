...
KW_TOPIC.5: /topic(?![\p{L}\p{N}_-])/i
KW_GUIDELINES.5: /guidelines/i
KW_SHARED.5: /shared(?![\p{L}\p{N}_-])/i
KW_IDENTIFIES.5: /identifies(?![\p{L}\p{N}_-])/i
KW_REFERS.5: /refers(?![\p{L}\p{N}_-])/i
KW_TO.5: /to(?![\p{L}\p{N}_-])/i
KW_ON.5: /on(?![\p{L}\p{N}_-])/i
```
...
### Lookahead de fronteira

Algumas keywords trazem o sufixo `(?![\p{L}\p{N}_-])` — um *negative lookahead* que impede o token de casar como **prefixo de um nome maior**. Sem ele, um campo chamado `topic_principal` teria seu início consumido pela keyword `TOPIC`, e o restante (`_principal`) viraria lixo sintático.

O cuidado é especialmente necessário nas palavras curtas introduzidas pela ligação entre projetos: `TO` e `ON` casariam dentro de nomes comuns como `to_do`, `total`, `on_hold` ou `onset`. Com o lookahead, `FIELD total TYPE TEXT` continua sendo um nome de campo, não a keyword `TO` seguida de `tal`.
...
include_type: KW_SHARED? (KW_BIBLIOGRAPHY | KW_ANNOTATIONS | KW_ONTOLOGY)
```
...
::: {.callout-note}
## Por que a gramática é permissiva com `SHARED`

`KW_SHARED?` aparece antes de **qualquer** tipo de inclusão, embora `SHARED` só seja válido para `ONTOLOGY`. A restrição é **semântica**, não sintática: `INCLUDE SHARED BIBLIOGRAPHY "..."` faz o parse normalmente e depois recebe o erro 84, com mensagem explicando por que a exceção existe apenas para ontologia.

Codificar a restrição na gramática produziria um "token inesperado" cru — sintaticamente correto, mas pedagogicamente inútil. A escolha é deliberada: **erro de gramática não ensina; erro semântico sim.**
:::
...
requirement_clause: KW_REQUIRED bundle_modifier? field_names
                  ...
                  | KW_REQUIRED field_key KW_ON KW_BIBLIOGRAPHY
...
O sufixo `ON BIBLIOGRAPHY` declara que o valor do campo vem da entrada `.bib` do SOURCE, não do texto do documento:
...
Note que as duas alternativas com `ON BIBLIOGRAPHY` usam `field_key` (singular), não `field_names` (lista). Isso é deliberado: numa lista, `REQUIRED a, b, c ON BIBLIOGRAPHY` seria ambíguo — o sufixo se aplicaria a `c` apenas, ou aos três? A gramática resolve a ambiguidade eliminando o caso: **lista + `ON BIBLIOGRAPHY` é erro de sintaxe.**
...
### Modificadores de ligação

`IDENTIFIES` e `REFERS TO` declaram a participação do campo na ligação entre projetos independentes:

```synesis
FIELD lattes_id TYPE TEXT
    SCOPE SOURCE
    IDENTIFIES researcher      # chave primária da entidade
END FIELD

FIELD autor_id TYPE TEXT
    SCOPE SOURCE
    REFERS TO researcher       # chave estrangeira para a entidade
END FIELD
```

O rótulo da entidade (`entity_label`) reusa o token `FIELD_NAME` — mesma classe léxica de um nome de campo. Como qualquer propriedade em `field_props`, os dois modificadores são aceitos pela gramática em campos de **qualquer** escopo; a restrição a `SCOPE SOURCE` é semântica (erro 78), pela mesma razão pedagógica do `SHARED`.

Ver [Ligação entre Projetos](guia_referencia.qmd) no Guia de Referência para a semântica completa.
